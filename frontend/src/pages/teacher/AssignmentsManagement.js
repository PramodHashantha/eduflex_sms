...
import { assignmentsAPI, classesAPI, enrollmentsAPI, usersAPI } from '../../services/api';
...
const AssignmentsManagement = () => {
  ...
  const fetchAssignments = async () => {
    setLoading(true);
    try {
      // Teacher can only see assignments for their classes
  ...
  const fetchStudentsForClass = async (classId) => {
    try {
      const enrollments = await enrollmentsAPI.getAll({ class: classId, status: 'active', isDeleted: 'false' });
  ...
  const handleViewMarks = async (assignment) => {
    ...
    await fetchStudentsForClass(assignment.class?._id || assignment.class);
